...
# cursor.execute('SELECT CIF_NO, CLIENT_TYPE FROM xxx.CLIENT')
...
# region xxx
    if report_type == 'test_report':
        ...
        query = "SELECT  " \
                ...
                "B.PRODUCT_SHORT_NAME + '-' + (CASE WHEN A.ACCOUNT_STATUS_CODE = DBO.GET_RPT_VALUE('testval')  " \
                "THEN 'NPL' WHEN A.ACCOUNT_STATUS_CODE = DBO.GET_RPT_VALUE('testval') THEN 'CURRENT' ELSE  " \
                "DBO.GET_DESC(A.ACCOUNT_STATUS_CODE,'testval') END) + '-' + A.TRAN_CODE AS 'SHEET NAME', " \
                "DBO.GET_DESC(A.ACCOUNT_STATUS_CODE,'testval') AS 'LOAN STATUS',  " \
                ...
                "CASE   " \
                "WHEN A.TRAN_CODE = 'test_code' THEN 0 " \
                ...
                "ISNULL(A.AR, 0) AS AR,  " \
                "CASE " \
                "WHEN A.TRAN_CODE = 'test_code' AND E.PRODUCT_CODE = 'testcode' THEN A.INTEREST " \
                "ELSE ISNULL(A.CCA, 0) " \
                "END AS CCA,  " \
                "CASE " \
                "WHEN A.TRAN_CODE = 'test_code' AND E.PRODUCT_CODE <> 'testcode' THEN A.INTEREST "  \
...
# region xxx_09
        ...
        query = "SELECT " \
                "B.PRODUCT_NAME AS 'LOAN TYPE', " \
                "B.PRODUCT_SHORT_NAME + '-' + (CASE WHEN A.ACCOUNT_STATUS_CODE = DBO.GET_RPT_VALUE('testval') THEN 'NPL' WHEN A.ACCOUNT_STATUS_CODE = DBO.GET_RPT_VALUE('LOAN_STATUS1') THEN 'CURRENT' ELSE DBO.GET_DESC(A.ACCOUNT_STATUS_CODE,'LOAN_STATUS') END) + '-' + A.TRAN_CODE AS 'SHEET NAME', " \
...
# region xxx_10
        ...
        query = "SELECT " \
                "PD.PRODUCT_SHORT_NAME + '-' + IIF(LHC.ACCOUNT_STATUS_CODE = DBO.GET_RPT_VALUE('testval'),'NPL',DBO.GET_DESC(LHC.ACCOUNT_STATUS_CODE,'testval')) AS 'SHEET NAME' " \
                ",DBO.GET_DESC(LHC.PRODUCT_CODE, 'testval') AS 'LOAN TYPE' " \
                ",DBO.GET_DESC(LHC.ACCOUNT_STATUS_CODE, 'testval') AS 'LOAN STATUS' " \
                ",LHC.TRAN_CODE AS 'TRAN CODE' " \
                ",LHC.TRAN_CODE_DESCRIPTION AS 'DESCRIPTION'	 " \
                ",COUNT(LHC.TRAN_CODE) AS 'NO OF RECORDS' " \
                ",SUM(IIF(ITEMS.TRAN_TYPE = DBO.get_val('testval') OR LHC.TRAN_CODE IN ('some values'), ISNULL(PRINCIPAL,0),0)) AS 'DEBIT' " \
                ",SUM(IIF(ITEMS.TRAN_TYPE = DBO.get_val('testval') OR LHC.TRAN_CODE IN ('some values') ,ISNULL(PRINCIPAL,0),0)) AS 'CREDIT' " \
                ...
                "AND (ITEMS.TRAN_TYPE <> (SELECT RPT_VALUE FROM RPT.REPORT_VARIABLE WITH(NOLOCK) WHERE RPT_CODE = 'test_code') OR LHC.TRAN_CODE = 'testcode') " \
